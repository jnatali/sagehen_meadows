#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
"""
# --- DUNDERS ---
__author__ = 'Kara-Leah Smittle, Jennifer Natali'
__copyright__ = 'Copyright (C) 2026'
__license__ = 'NOT Licensed, Private Code under Development, DO NOT DISTRIBUTE'
__maintainer__ = 'Jennifer Natali'
__email__ = 'jennifer.natali@berkeley.edu'
__status__ = 'Development'

# ---- IMPORTS ---- 
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import numpy as np
import os
from well_utils import get_well_categories

# ---- GLOBAL VARIABLES ---- 

## --- SETUP DIRECTORY + FILE NAMES --- 

## --- SETUP FLAGS 
##     for processing, logging, debugging and data validation
SOURCE="14d"
EXCLUDE_2025 = False
FILL_SINGLE_BIN_GAPS = True

### --- INPUTS
SOURCE_DIR = os.path.join('..', '..', 'data', 'field_observations', 'groundwater', 'time_series')
SOURCE_FILE_WEEKLY_MATRIX = "groundwater_weekly_matrix.csv"
SOURCE_FILE_14D_BIN = "groundwater_bin_16d.csv"


### --- OUTPUTS
# Create the directory for saving files
output_dir = os.path.join('..', '..', 'results', 'plots', 'groundwater', 'mean')
os.makedirs(output_dir, exist_ok=True)
output_dir = output_dir + os.sep 


# ---- DEFINE FUNCTIONS ---- 

def filter_year_exclusion(df) -> pd.DataFrame:
    
    if EXCLUDE_2025:
        df = df[df['year'] != 2025]
    
    return df

def interpolate_single_bin_gaps(
    df: pd.DataFrame,
    value_col: str = "level",
    bin_col: str = "bin",
    group_cols=("well_id", "year"),
    ) -> pd.DataFrame:
    """
    Linearly interpolate single-bin gaps only, per well and year.
    Longer gaps are left as NaN.
    """

    def _interpolate_group(g):
        g = g.sort_values(bin_col).copy()

        # Identify missing values
        is_na = g[value_col].isna()

        if not is_na.any():
            return g

        # Linear interpolation (temporary)
        interp = g[value_col].interpolate(method="linear")

        bin_prev = g[bin_col].shift(1)
        bin_next = g[bin_col].shift(-1)
        
        prev_ok = ~is_na.shift(1).fillna(False) & (bin_prev == g[bin_col] - 1)
        next_ok = ~is_na.shift(-1).fillna(False) & (bin_next == g[bin_col] + 1)
        
        single_gap = is_na & prev_ok & next_ok
        
        g.loc[single_gap, value_col] = interp.loc[single_gap]

        return g

    return (
        df
        .groupby(list(group_cols), group_keys=False)
        .apply(_interpolate_group)
    )

def process_weekly_matrix(df) -> pd.DataFrame:
    # Identify which columns are ID and which are Data (Weeks)
    id_cols = [c for c in df.columns if not str(c).startswith('20')]
    week_cols = [c for c in df.columns if str(c).startswith('20')]
    
    # ---------------------------------------------------------
    # 3. MELT THE DATA (The Critical Step)
    # ---------------------------------------------------------
    # 'Melting' is the process of unpivoting a DataFrame from 
    # Wide format (human-readable) to Long format (computer-readable).
    #
    # CURRENT STATE (Wide):
    # well_id | 201801 | 201802 | ...
    # W-001   |  10.5  |  12.1  | ...
    #
    # DESIRED STATE (Long):
    # well_id | ISO_Week | Level
    # W-001   | 201801   | 10.5
    # W-001   | 201802   | 12.1
    #
    # EXPLANATION OF ARGUMENTS:
    # id_vars:    The columns you want to KEEP as identifiers. 
    #             These values will repeat for every week. (e.g., well_id)
    # value_vars: The columns you want to UNPIVOT. 
    #             These headers (201801, etc.) will vanish from the top 
    #             and move into a new column rows.
    # var_name:   New column that contains the 
    #             iso week column. We call it 'ISO_Week_Raw' because it contains 
    #             the raw string (e.g., "201801") which we must split later.
    # value_name: New column that contains the 
    #             grounwater levels, called 'Level'.
    
    df_long = df.melt(
        id_vars=id_cols, 
        value_vars=week_cols, 
        var_name='ISO_Week_Raw', 
        value_name='Level'
    )
    # ---------------------------------------------------------
    
    # 4. Clean Up Dates
    # We parse the '201801' string into Year (2018) and Week (1).
    df_long['Year'] = df_long['ISO_Week_Raw'].astype(str).str[:4].astype(int)
    df_long['Week'] = df_long['ISO_Week_Raw'].astype(str).str[4:].astype(int)
    
    # 5. Categorization Logic (Updated to match Canopy Script)
    # Explicitly using 'well_id' as requested.
    well_id_col = 'well_id' 
    
    # Safety check: ensure the column actually exists before proceeding
    if well_id_col not in df_long.columns:
        raise ValueError(f"Column '{well_id_col}' not found in CSV")
    
    df_long = get_well_categories(df_long)
    df_long = filter_year_exclusion(df_long)
    # TODO: Assign bin and DOY
    
    return df_long

def process_14d_bin(df, fill_gaps=FILL_SINGLE_BIN_GAPS) -> pd.DataFrame:
    """
    Processes the 14-day binned groundwater data immediately 
    after loading from csv    
    
    Parameter:
        df has columns ['well_id', 
                        'timestamp',
                        'ground_to_water',
                        'year',
                        'doy',
                        'bin']
        
    Returns processed dataframe with column renaming and well categorization added
    """

    bin_df = df.copy()
    
    bin_df = get_well_categories(df)
    bin_df = bin_df.rename(columns={'ground_to_water_cm': 'level'})
    bin_df = filter_year_exclusion(bin_df)
    
    if fill_gaps:
        bin_df = interpolate_single_bin_gaps(
        bin_df,
        value_col="level",
        bin_col="bin",
        group_cols=("well_id", "year"),
        )
    
    return bin_df

def load_source_data() -> pd.DataFrame:
    if SOURCE == "weekly_matrix":
        source_file = SOURCE_FILE_WEEKLY_MATRIX
        file_path = os.path.join(SOURCE_DIR, source_file)
        source_df = pd.read_csv(file_path)
        source_df = process_weekly_matrix(source_df)
        return source_df
    
    elif SOURCE == "14d":
        source_file = SOURCE_FILE_14D_BIN
    else:
        print("Define input DATA source, default to 14d bin.")
        print("See SOURCE global variable to change.")
        source_file = SOURCE_FILE_14D_BIN
    
    file_path = os.path.join(SOURCE_DIR, source_file)
    source_df = pd.read_csv(file_path)
    source_df = process_14d_bin(source_df)
    
    return source_df

def plot_well_annual_14d_bin(
    df: pd.DataFrame,
    year_palette: dict,
    x_col: str = "doy",
    y_col: str = "level",
    show: bool = False
    ):
    """
    Create one plot per well showing all years of 14-day binned groundwater data.

    Saves both .eps and .pdf and optionally displays each plot.

    Parameters
    ----------
    df : pandas.DataFrame
        Must contain columns:
        ['well_id', 'year', x_col, y_col]
    out_dir : str
        Directory where plots will be saved
    year_palette : dict
        Mapping {year: color}
    x_col : str
        DOY-like x-axis column (default 'bin_center_doy')
    y_col : str
        Groundwater column (default 'level')
    show : bool
        Whether to display plots interactively
    """
    out_path = os.path.join(output_dir,'well_plots/')
    os.makedirs(out_path, exist_ok=True)

    required = {"well_id", "year", x_col, y_col}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Missing required columns: {missing}")

    for well_id, g_well in df.groupby("well_id"):

        fig, ax = plt.subplots(figsize=(8, 4))

        for year, g_year in g_well.groupby("year"):
            color = year_palette.get(year, "gray")

            g_year = g_year.sort_values(x_col)

            ax.plot(
                g_year[x_col],
                g_year[y_col],
                marker="o",
                linewidth=1.8,
                markersize=4,
                color=color,
                label=str(year),
            )

        ax.set_title(f"{well_id} — 14-day binned groundwater")
        ax.set_xlabel("Day of Year")
        ax.set_ylabel("Groundwater depth (cm below ground)")


        ax.invert_yaxis()

        ax.grid(alpha=0.3)
        ax.legend(title="Year", ncol=2, fontsize=9)

        fig.tight_layout()

        # ---- Save files
        base_pdf = os.path.join(out_path, f"{well_id}_annual_14d_bin.pdf")
        base_eps = os.path.join(out_path, f"{well_id}_annual_14d_bin.eps")
        fig.savefig(base_pdf, format="pdf")
        fig.savefig(base_eps, format="eps")

        if show:
            plt.show()
        else:
            plt.close(fig)
    return
    

def plot_annual_mean_all_wells(df_long, YEAR_PALETTE):
    # PLOT MEAN LEVELS ACROSS ALL WELLS ---

    # Calculate means so we can use them to set the axis limits
    yearly_means = df_long.groupby(['year', 'doy'])['level'].mean().reset_index()
    grand_mean = df_long.groupby('doy')['level'].mean().reset_index()
    # JN 02/17/2026: eliminate spurious first point due to lack of data
    grand_mean = grand_mean[grand_mean['doy'] > 140]
    
    t1_max = yearly_means['level'].max() + 5
    t1_min = yearly_means['level'].min() - 5

    # 1. INDIVIDUAL PLOT: Annual Lines
    plt.figure(figsize=(10, 6))
    sns.lineplot(data=yearly_means, x='doy', y='level', hue='year', palette=YEAR_PALETTE, marker='o')
    plt.title('Mean GW Level by Year (All Wells)')
    plt.ylabel('Depth Below Surface (cm)')
    plt.grid(True, alpha=0.3)
    # FIX: Use (Deepest, Shallowest) to invert manually
    plt.ylim(t1_max, t1_min) 
    #plt.savefig(f"{output_dir}Mean_GW_Level_by_Year.eps", format='eps')
    plt.savefig(f"{output_dir}Mean_by_year.eps", format='eps')
    plt.savefig(f"{output_dir}Mean_by_year.pdf", format='pdf')
    # plt.show()

    # 2. INDIVIDUAL PLOT: Grand Mean Only
    plt.figure(figsize=(10, 6))
    sns.lineplot(data=grand_mean, x='doy', y='level', color='black', linewidth=3, label='Grand Mean')
    plt.title('Grand Mean (All Years Aggregated)')
    plt.ylabel('Depth Below Surface (cm)')
    plt.xlabel('Day of Year')
    plt.grid(True, alpha=0.3)
    # FIX: Use (Deepest, Shallowest) to invert manually
    plt.ylim(t1_max, t1_min) 
    #plt.savefig(f"{output_dir}GRAND_Mean_GW_Level_by_Year.eps", format='eps')
    plt.savefig(f"{output_dir}Mean_by_year_grand_only.eps", format='eps')
    plt.savefig(f"{output_dir}Mean_by_year_grand_only.pdf", format='pdf')
    # plt.show()

    # 3. COMBINED PLOT: Annual + Grand Mean Overlay
    plt.figure(figsize=(10, 6))
    # Plot annual lines with lower transparency (alpha) so the Grand Mean stands out
    sns.lineplot(data=yearly_means, x='doy', y='level', hue='year', palette=YEAR_PALETTE, alpha=0.4, legend=False)
    # Overlay the Grand Mean
    sns.lineplot(data=grand_mean, x='doy', y='level', color='black', linewidth=4, label='Grand Mean')
    plt.title('Combined: Annual Trends vs. Grand Mean (All Wells)')
    plt.ylabel('Depth Below Surface (cm)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    # FIX: Use (Deepest, Shallowest) to invert manually
    plt.ylim(t1_max, t1_min) 
    #plt.savefig(f"{output_dir}COMBINED_Mean_GW_Level_by_Year.eps", format='eps')
    plt.savefig(f"{output_dir}Mean_by_Year_grand.eps", format='eps')
    plt.savefig(f"{output_dir}Mean_by_Year_grand.pdf", format='pdf')

    plt.show() # Pausing here so you can check Task 1
    plt.close()
    print("Plotted annual means")
    return

def plot_annual_mean_std_multiplot(df_long, YEAR_PALETTE):
    # --- TASK 4: VISUALIZING SPREAD (StDev) ---

    # Calculate Mean and Standard Deviation per week
    stats = df_long.groupby(['year', 'doy'])['level'].agg(['mean', 'std']).reset_index()

    # --- CALCULATE LIMITS SPECIFICALLY FOR TASK 4 ---
    # We must account for the standard deviation spread so the shading fits
    t4_max = (stats['mean'] + stats['std']).max() + 5
    t4_min = (stats['mean'] - stats['std']).min() - 5

    # Create a 2x2 grid of subplots (for the 4 years)
    # sharex/sharey ensures they all use the exact same scale for comparison
    fig, axs = plt.subplots(2, 2, figsize=(14, 10), sharex=True, sharey=True)
    axs = axs.flatten()  # Flatten the 2D grid into a 1D list for easier looping

    unique_years = sorted(stats['year'].unique())

    for i, year in enumerate(unique_years):
        # Safety check in case there are more than 4 years in data
        if i >= len(axs): break
        
        ax = axs[i]
        subset = stats[stats['year'] == year]
        
        # Retrieve the consistent color for this year
        c = YEAR_PALETTE.get(year, 'black')
        
        # Plot line
        ax.plot(subset['doy'], subset['mean'], label=str(year), color=c, linewidth=2)
        
        # Plot spread (Mean +/- StdDev)
        ax.fill_between(
            subset['doy'], 
            subset['mean'] - subset['std'], 
            subset['mean'] + subset['std'], 
            color=c, alpha=0.15
        )
        
        # Subplot specific styling
        ax.set_title(f"Year: {year}", fontweight='bold')
        ax.grid(True, alpha=0.3)

    # Axis Handling (Applied to all subplots via sharey/sharex)
    # We set the limits on the first axis, and it propagates
    # FIX: Explicitly set (Deepest, Shallowest) to force inversion
    axs[0].set_ylim(t4_max, t4_min)  

    # Global Labels (placed on the figure object to avoid clutter)
    fig.supylabel('Depth Below Surface (cm)')
    fig.supxlabel('Day of Year')

    plt.tight_layout()

    # Save
    #save_path = f"{output_dir}annual_means_with_std_spread_subplots.eps"
    save_path_eps = f"{output_dir}Mean_by_year_std_spread.eps"
    save_path_pdf = f"{output_dir}Mean_by_year_std_spread.pdf"
    plt.savefig(save_path_eps, format='eps')
    plt.savefig(save_path_pdf, format='pdf')
    plt.show()
    plt.close()
    print("Plotted annual means multiplot")
    
    return

def plot_category_means(df_long, YEAR_PALETTE):
    categories_to_plot = ['meadow_id', 'plant_type', 'hydrogeo_zone']

    for category_col in categories_to_plot:
        
        # Get valid unique values
        unique_cats = [c for c in df_long[category_col].unique() if c is not None]
        num_cats = len(unique_cats)
        
        # ---------------------------------------------------------
        # PART 1: PLOT SEPARATELY (Individual files)
        # ---------------------------------------------------------
        for cat_val in unique_cats:
            # Prepare Data
            subset = df_long[df_long[category_col] == cat_val]
            subset_annual = subset.groupby(['year', 'doy'])['level'].mean().reset_index()
            
            # Plot
            plt.figure(figsize=(8, 5))
            sns.lineplot(data=subset_annual, x='doy', y='level', hue='year', palette=YEAR_PALETTE, marker='.')
            
            # Formatting
            plt.title(f'Individual: {cat_val}')
            plt.grid(True, alpha=0.3)
            plt.ylabel('Depth Below Surface (cm)')
            
            # FIX: Manual inversion for single plots
            plt.gca().invert_yaxis() 
            
            # Save
            clean_name = str(cat_val).replace(" ", "_")
            plt.savefig(f"{output_dir}SEPARATE_{category_col}_{clean_name}.eps", format="eps")
            plt.savefig(f"{output_dir}SEPARATE_{category_col}_{clean_name}.pdf", format="pdf")
            plt.close() 

        # ---------------------------------------------------------
        # PART 2: PLOT TOGETHER (Subplots)
        # ---------------------------------------------------------
        # Create grid
        fig, axes = plt.subplots(1, num_cats, figsize=(5 * num_cats, 6), sharey=True)
        if num_cats == 1: axes = [axes] # Handle single-item case

        # Global scale for this comparison
        cat_subset = df_long[df_long[category_col].isin(unique_cats)]
        t2_max = cat_subset['level'].max() - 80
        t2_min = cat_subset['level'].min()

        for i, cat_val in enumerate(unique_cats):
            ax = axes[i]
            type_data = df_long[df_long[category_col] == cat_val]
            
            # Calculate means
            type_annual = type_data.groupby(['year', 'doy'])['level'].mean().reset_index()
            
            # --- NEW LOGIC: Calculate Non-Drought Mean ---
            # Filter out 2021 first, then group by Week
            non_drought_data = type_data[type_data['year'] != 2021]
            non_drought_mean = non_drought_data.groupby('doy')['level'].mean().reset_index()

            # A. Plot Background Years (Non-Drought Years)
            if not non_drought_data.empty:
                # Alpha increased to 0.3 as requested
                sns.lineplot(data=type_annual[type_annual['year'] != 2021], 
                             x='doy', y='level', hue='year', palette=YEAR_PALETTE, 
                             ax=ax, alpha=0.3, legend=False, linewidth=1.5)
            
            # B. Plot 2021 (Drought Year)
            drought_year = type_annual[type_annual['year'] == 2021]
            if not drought_year.empty:
                sns.lineplot(data=drought_year, x='doy', y='level', color=YEAR_PALETTE.get(2021, 'red'), 
                             ax=ax, linewidth=3, label='2021 (Drought)' if i == num_cats-1 else None)
            
            # C. Plot Non-Drought Mean (Black, Dashed)
            sns.lineplot(data=non_drought_mean, x='doy', y='level', color='black', 
                         ax=ax, linewidth=2, linestyle='--', label='Non-Drought Avg' if i == num_cats-1 else None)
            
            # Formatting
            ax.set_title(cat_val, fontweight='bold')
            
            # FIX: Explicitly set (Deepest, Shallowest) to force inversion
            ax.set_ylim(t2_max, t2_min) 
            
            ax.grid(True, alpha=0.2)
            
            if i == 0:
                ax.set_ylabel('Depth Below Surface (cm)')
            else:
                ax.set_ylabel('')

        # Unified Title
        plt.suptitle(f'Comparison by {category_col}: 2021 vs Non-Drought Average', fontsize=16, y=1.02)
        plt.tight_layout()
        
        # Save
        plt.savefig(f"{output_dir}TOGETHER_Grid_{category_col}.eps", bbox_inches='tight', format='eps')
        plt.savefig(f"{output_dir}TOGETHER_Grid_{category_col}.pdf", bbox_inches='tight', format='pdf')
        plt.show()
        plt.close()
    
    print("Plotted category means")
    return

def plot_drought_means(df_long, YEAR_PALETTE):

    drought_year = 2021
    # Mark years as Drought vs Non-Drought
    # This logic automatically buckets 2018, 2019, 2024, etc. as Non-Drought
    df_long['Status'] = df_long['year'].apply(lambda y: 'Drought (2021)' if y == drought_year else 'Non-Drought')
    status_means = df_long.groupby(['Status', 'doy'])['level'].mean().reset_index()
    
    # --- CALCULATE LIMITS SPECIFICALLY FOR TASK 3 ---
    t3_max = status_means['level'].max() + 5
    t3_min = status_means['level'].min() - 5
    
    plt.figure(figsize=(12, 6))
    
    # Plot Non-Drought (Average of all other years)
    sns.lineplot(
        data=status_means[status_means['Status']=='Non-Drought'], 
        x='doy', y='level', color='grey', label='Non-Drought Average', linewidth=3
    )
    
    # Plot Drought Year
    sns.lineplot(
        data=status_means[status_means['Status']=='Drought (2021)'], 
        x='doy', y='level', color='red', label='Drought (2021)', linewidth=3
    )
    
    # Shade the difference
    pivoted = status_means.pivot(index='doy', columns='Status', values='level')
    plt.fill_between(
        pivoted.index, 
        pivoted['Non-Drought'], 
        pivoted['Drought (2021)'], 
        color='red', alpha=0.1
    )
    
    plt.title('Drought Impact: 2021 vs Average of other years')
    plt.ylabel('Depth Below Surface (cm)')
    plt.grid(True, alpha=0.3)
    
    # Axis Handling
    # FIX: Explicitly set (Deepest, Shallowest) to force inversion
    plt.ylim(t3_max, t3_min) 
    
    # Save
    #save_path = f"{output_dir}drought_comparison_2021.eps"
    save_path_eps = f"{output_dir}Mean_by_year_drought.eps"
    save_path_pdf = f"{output_dir}Mean_by_year_drought.pdf"
    plt.savefig(save_path_eps, format='eps')
    plt.savefig(save_path_pdf, format='pdf')

    
    plt.show()
    plt.close()
    print("Plotted drought means")

    return

# def categorize_well(val):
#     # Define the valid mapping dictionaries
#     meadow_map = {'K': "Kiln", 'E': "East"}
#     plant_map = {'E': "Sedge", 'W': "Willow", 'H': "Mixed Herbaceous"}
#     zone_map = {'R': "Riparian", 'T': "Terrace", 'F': "Fan"}

#     try:
#         # Extract the prefix (e.g., 'KWR')
#         codes = str(val).strip().split('-')[0]
        
#         # Ensure we have at least 3 characters to unpack
#         if len(codes) < 3:
#             return pd.Series([None, None, None])
        
#         m_code, p_code, z_code = codes[0], codes[1], codes[2]
        
#         # Validation: Check if ALL codes exist in our maps
#         if (m_code in meadow_map and 
#             p_code in plant_map and 
#             z_code in zone_map):
            
#             return pd.Series([
#                 meadow_map[m_code], 
#                 plant_map[p_code], 
#                 zone_map[z_code]
#             ])
#         else:
#             # If any code is unrecognized, return None
#             return pd.Series([None, None, None])

#     except Exception:
#         # Catch-all for malformed strings or unexpected errors
#         return pd.Series([None, None, None])

# Apply logic specifically to the 'well_id' column
# df_long[['Site', 'Plant_Type', 'Zone']] = df_long[well_id_col].apply(categorize_well)
# df_long = df_long.dropna(subset=['Site', 'Plant_Type', 'Zone'])
# df_long['Level'] = pd.to_numeric(df_long['Level'], errors='coerce')

# (Removed Global Y-Axis Limits Section here because we will calculate them 
# dynamically inside each task to avoid the "off-screen" issue)

# --- MAIN ---
# Define a main() function to allow import of functions
# without executing the full script.

def main():
    
    df = load_source_data()
    
    # Create a dictionary that maps every Year to a specific Color.
    # This ensures that "2018" is always Blue (for example), even if
    # 2018 is missing from some specific graphs.
    unique_years = sorted(df['year'].unique())
    year_palette = dict(zip(unique_years, sns.color_palette("tab10", len(unique_years))))
    
    plot_well_annual_14d_bin(df, year_palette)
    plot_annual_mean_all_wells(df, year_palette)
    plot_annual_mean_std_multiplot(df, year_palette)
    plot_category_means(df, year_palette)
    plot_drought_means(df, year_palette)
    

# ---- Execute MAIN ----
# allows import of functions into another script
# without executing this full script.

if __name__ == "__main__":
    main()