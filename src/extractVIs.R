extractVIs <- function(img.path,roi.path,vi.path=NULL,roi.name=NULL,plot=TRUE, begin=NULL, spatial=FALSE, date.code, npixels=1, 
  ...
  if (spatial==FALSE) {
    ...
    for (roi in roi.pos) {   
      write_log(paste("roi num", roi, "start\n"))
      ...
      # registerDoParallel(cl)
      write_log(paste("roi num", roi, "before file loop\n"))
      img <- NULL    
      VI.data.roi <- foreach(img=1:n_files, .packages=c('raster', 'phenopix'), .combine=rbind) %dopar% {

        write_log(paste("start img", roi, img, n_files, '\n'))
        ...
        write_log(paste("end img", roi, img, n_files, '\n'))
        temp.VI
      } #endfor loop images  

      write_log(paste("roi num", roi, "after file loop\n"))
      stopCluster(cl)
      ...
      write_log(paste("roi num", roi, "before plot\n"))
      ...
      write_log(paste("roi num", roi, "after plot\n"))
    } #endfor loop rois
